...
/* fun to initialize Memory Manager */
void mm_init()
{
    SYSTEM_PAGE_SIZE = getpagesize();
}

/* Fun to get max page allocated memory  */
...
vm_page_t* allocate_vm_page(vm_page_family_t *vm_page_family)
{
    ...
    init_gl_dbl_lkdlist(&vm_page->block_meta_data.priority_thread_glue);
    vm_page->next = NULL;
    vm_page->prev = NULL;
...
/* fun to display VM page details */
    ...
    uint32_t struct_size)
{
    ...
    if (!first_vm_page_for_families)
    {
        ...
        init_gl_dbl_lkdlist(&first_vm_page_for_families->vm_page_family[0].free_block_priority_list_head);
        return;
    }
    ...
    init_gl_dbl_lkdlist(&vm_page_family_curr->free_block_priority_list_head);
}

/* Funt to display registered page families */
    ...
    block_meta_data_t *free_block)
{
    ...
    gl_dbl_lkdlist_priority_insert(&vm_page_family->free_block_priority_list_head,
    ...
    uint32_t size)
{
    ...
    block_meta_data->block_size = size;
    remove_gl_dbl_lkdlist(&block_meta_data->priority_thread_glue);
             ...
             remaining_size < (sizeof(block_meta_data_t) + vm_page_family->struct_size))
    {
        ...
        init_gl_dbl_lkdlist(&next_block_meta_data->priority_thread_glue);
        mm_add_free_block_meta_data_to_free_block_list(
...
void mm_print_block_usage()
{
    ...
    ITERATE_PAGE_FAMILIES_BEGIN(first_vm_page_for_families, vm_page_family_curr)
    {
        ...
        printf("%-20s   Total Block : %-4u    Free Block : %-4u    Occupied Block : %-4u AppMemUsage : %u\n",
...
/* fun to display total memory usage */
void mm_print_memory_usage(char *struct_name)
{
    ...
    printf(ANSI_COLOR_MAGENTA "Number Of VM Pages in Use : %u (%lu Bytes)\n" ANSI_COLOR_RESET,
           cumulative_vm_pages_claimed_from_kernel,
